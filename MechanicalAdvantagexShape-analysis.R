...
#########################################
## INTERPLAY CLAW EFFICIENCY AND SHAPE ##
#########################################

beta1<-betareg(maT~proc.scale*species*sex | species*sex,data=full.table)
...
beta2<-betareg(maT~proc.scale*species+species*sex+lncs.scale*sex | species*sex,data=full.table)
beta3<-betareg(maT~proc.scale*species+species*sex+sex | species*sex,data=full.table)
beta4<-betareg(maT~proc.scale*species+species+lncs.scale*sex | species*sex,data=full.table)
...
#look in summary(beta1)

betaEST<-betareg(maT~proc.scale*species*sex-1 | species*sex,data=full.table)
...
exp(beta.coef[1]+confint(betaEST)[8,1])/(1+exp(beta.coef[1]+confint(betaEST)[8,1])) #SLOPE 2.5%
exp(beta.coef[1]+confint(betaEST)[8,2])/(1+exp(beta.coef[1]+confint(betaEST)[8,2])) #SLOPE 97.5%
...
betaPlot<-betareg(maT~dist.proc*species*sex | species*sex,data=full.table)
...
#tiff(file="maTxProcDist.tiff",units="mm",width=170,height=150,res=600,
#     compression="lzw")
plot(maT~dist.proc,pch=c(24,21)[as.numeric(sex)],
...
legend(0.08,0.65, legend=c(expression(italic("Aegla abtao"),
       ...
       cex=1.2,ncol=2)
legend("topright",legend="(e)",cex=1.2,bty='n')
...
lines(dentM$dist.proc,predict(betaPlot,type='response',newdata=dentM),lwd=3,col="yellow4")
...
lines(dentF$dist.proc,predict(betaPlot,type='response',newdata=dentF),lwd=3,lty=2,col="yellow4")
...
lines(abtaoM$dist.proc,predict(betaPlot,type='response',newdata=abtaoM),col="black",lwd=3)
...
lines(abtaoF$dist.proc,predict(betaPlot,type='response',newdata=abtaoF),col="black",lwd=3,lty=2)
...
lines(longM$dist.proc,predict(betaPlot,type='response',newdata=longM),col="darkred",lwd=3)
...
lines(longF$dist.proc,predict(betaPlot,type='response',newdata=longF),col="darkred",lwd=3,lty=2)
